function results = affine_batch_experiment_shear (keypoint_detector, descriptor_extractors, varargin)
    % results = AFFINE_BATCH_EXPERIMENT_SHEAR (keypoint_detector, descriptor_extractors, varargin)
    %
    % Batch experiments on a sheared image from Oxford Affine dataset.
    %
    % Input:
    ...
    %     - shear_values: shear in both X and Y direction (default: -0.65:0.05:0.65)
    ...
    num_shear_values = numel(shear_values);
    
    %% Dataset
    dataset = AffineDataset();

    %% Process all image pairs
    recognition_rates = nan(num_repetitions, size(descriptor_extractors, 1), num_shear_values);
    num_keypoints1 = nan(num_shear_values, 1);
    num_keypoints2 = nan(num_shear_values, 1);
    num_established_correspondences = nan(num_shear_values, 1);
    
    for i = 1:num_shear_values,
        shear = shear_values(i);

        fprintf('\n--- Shear: %d/%d: %f ---\n', i, num_shear_values, shear);

        % Generate image pair
        [ I1, I2, H12 ] = dataset.get_sheared_image(sequence, image, shear, shear);
    ...
    results.image = image;
    results.shear_values = shear_values;
