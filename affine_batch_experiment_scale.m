function results = affine_batch_experiment_scale (keypoint_detector, descriptor_extractors, varargin)
    % results = AFFINE_BATCH_EXPERIMENT_SCALE (keypoint_detector, descriptor_extractors, varargin)
    %
    % Batch experiments on a scaled image from Oxford Affine dataset.
    %
    % Input:
    ...
    %     - scales: array of scale factors (default: 1.00:0.05:1.50)
    ...
    %     - scales: copy of the input scales array
    ...
    parser.addParameter('scales', 1.00:0.05:1.50, @isnumeric);
    ...
    recognition_rates = nan(num_repetitions, size(descriptor_extractors, 1), numel(scales));
    num_keypoints1 = nan(num_scales, 1);
    num_keypoints2 = nan(num_scales, 1);
    num_established_correspondences = nan(num_scales, 1);
    
    for i = 1:num_scales,
        scale = scales(i);

        fprintf('\n--- Scale: %d/%d: %f ---\n', i, numel(scales), scale);

        % Generate image pair
        [ I1, I2, H12 ] = dataset.get_scaled_image(sequence, image, scale);
