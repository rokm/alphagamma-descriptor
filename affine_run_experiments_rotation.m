function affine_run_experiments_rotation (varargin)
    % AFFINE_RUN_EXPERIMENTS_ROTATION (varargin)
    %
    % Runs rotation experiments on Affine Dataset sequences.
    %
    ...
    parser.addParameter('result_dir', 'results-affine-rotation', @ischar);
    ...
    for e = 1:numel(experiments),
        ...
        for s = 1:numel(sequences),
            ...
            if ~exist(result_file, 'file'),
                result = affine_batch_experiment_rotation(experiment.keypoint_detector, experiment.descriptor_extractors, 'sequence', sequence, 'image', image, 'angles', angles, 'project_keypoints', project_keypoints);
            ...
            if visualize,
                visualize_results_rotation(result);
            end
        end 
    end
end