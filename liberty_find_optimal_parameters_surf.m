% LIBERTY_FIND_OPTIMAL_PARAMETERS_SURF ()
% 
% Determines optimal parameters for SURF descriptor (the scaling parameter
% for the patches).
function liberty_find_optimal_parameters_surf ()

    %% Settings
    settings.patch_scales = [ 1:20, 1/(9/1.2 * 1/(21)) ];
    ...
    surf = vicos.descriptor.SURF('HessianThreshold', 400, 'NOctaves', 3, 'NOctaveLayers', 4); % use OpenCV 2.3 options
    result_file = 'liberty-scale-optimization-surf.mat';
    
    if ~exist(result_file, 'file'),
        results = evaluate_descriptor_extractor_parameters(surf, settings);
    ...
    title('SURF recognition rate on liberty');
