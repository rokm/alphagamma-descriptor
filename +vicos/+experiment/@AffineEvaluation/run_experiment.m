function results = run_experiment (self, keypoint_detector, descriptor_extractor, sequence, varargin)
    ...
    results = repmat(struct('sequence', [], ...
                            ...
                            'num_correct_matches', [], ...
                            'num_keypoints_ref_unique', [], ...
                            'num_keypoints_test_unique', [], ...
    ...
    for i = 1:numel(test_images)
        ...
        results(i).num_keypoints_ref = numel(ref_keypoints_raw);
        results(i).num_keypoints_test = numel(test_keypoints_raw);
        ...
        % Filter out duplicates from raw keypoints
        tmp = [ vertcat(ref_keypoints_raw.pt), vertcat(ref_keypoints_raw.size) ];
        tmp = unique(tmp, 'rows'); % Unique keypoints in reference image
        results(i).num_keypoints_ref_unique = size(tmp, 1 );
        
        tmp = [ vertcat(test_keypoints_raw.pt), vertcat(test_keypoints_raw.size) ];
        tmp = unique(tmp, 'rows'); % Unique keypoints in test image
        results(i).num_keypoints_test_unique = size(tmp, 1 );
