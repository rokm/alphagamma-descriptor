function jasna_experiment_shear (varargin)
    ...
    alphagamma_float_opts  = { 'non_binarized_descriptor', true,  'num_rays', 13, 'num_circles',  9,  'circle_step', sqrt(2)*1.104, 'base_keypoint_size', base_keypoint_size(1) };
    alphagamma_short_opts  = { 'non_binarized_descriptor', false, 'num_rays', 23, 'num_circles', 10,  'circle_step', sqrt(2)*1.042, 'base_keypoint_size', base_keypoint_size(2) };
    ...
    if display_results
        figure;
        for s = 1:size(experiment_ids, 1)
            plot(shear_factors, recognition_rate{s}, 'LineWidth', 2.0); hold on;
        end
    ...
    if ~isempty(result_file)
        ...
        % Column names
        fprintf(fid, 'columns ');
        fprintf(fid, '%d ', 1:numel(shear_factors));
        fprintf(fid, '\n');
        
        % Units
        fprintf(fid, 'param ');
        fprintf(fid, '%g ', shear_factors);
        fprintf(fid, '\n');
